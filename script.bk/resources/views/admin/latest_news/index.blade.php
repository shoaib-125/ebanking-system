...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>'News List'])
@endsection
            ...
            <div class="row mb-4">
                ...
                <div class="col-lg-6">
                    <div class="add-new-btn">
                        <a href="{{ route('admin.latest_news.create') }}" class="btn btn-primary float-right">{{ __('Add New News') }}</a>
                    </div>
                </div>
            </div>
                      ...
                      <td>
                        <div class="dropdown d-inline">
                          ...
                          <div class="dropdown-menu">
                            <a class="dropdown-item has-icon" href="{{ route('admin.latest_news.edit', $row->id) }}"><i class="fa fa-edit"></i>{{ __('edit') }}</a>
                            <a class="dropdown-item has-icon delete-confirm" href="javascript:void(0)" data-id={{ $row->id }}><i class="fa fa-trash"></i>{{ __('Delete') }}</a>
                            <!-- Delete Form -->
                            <form class="d-none" id="delete_form_{{ $row->id }}" action="{{ route('admin.latest_news.destroy', $row->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            </form>
                          </div>
