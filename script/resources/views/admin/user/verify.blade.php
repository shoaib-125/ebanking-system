...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>'email unverified Users'])
@endsection
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-striped" id="table-2">
                  ...
                  <tbody>
                    @foreach($user_unverified as $row)
                    <tr>
                      <td>
                        <div class="custom-checkbox custom-control">
                      ...
                      <td>{{ $row->account_number }}</td>
                        ...
                        <td>
                            <a href="{{$row->id_front}}" target="_blank"><span class="badge badge-info">ID Image(Front)</span></a>
                            <a href="{{$row->id_back}}" target="_blank"><span class="badge badge-info">ID Image(Back)</span></a>
                            <a href="{{$row->selfie}}" target="_blank"><span class="badge badge-info">Selfie</span></a>
                            <a href="{{$row->bill}}" target="_blank"><span class="badge badge-info">Utility Bill</span></a>
                        </td>
                        ...
                        <div class="dropdown d-inline">
                          ...
                          <div class="dropdown-menu">
                            ...
                            <a class="dropdown-item has-icon" href="{{ url('/login') }}"><i class="fa fa-edit"></i>{{ __('Login') }}</a>
                              <a class="dropdown-item has-icon approve-confirm" href="javascript:void(0)" data-id={{ $row->id }}><i class="fa fa-edit"></i>{{ __('Verify') }}</a>
                              <a class="dropdown-item has-icon reject-confirm" href="javascript:void(0)" data-id={{ $row->id }}><i class="fa fa-trash"></i>{{ __('Reject') }}</a>
                            ...
                            <form class="d-none" id="approve_form_{{ $row->id }}" action="{{ route('admin.users.update.status', $row->id) }}" method="POST">
                                <input type="hidden" name="update_verify" value="true">
                            @csrf
                            </form>
                            <form class="d-none" id="reject_form_{{ $row->id }}" action="{{ route('admin.users.update.status', $row->id) }}" method="POST">
                                <input type="hidden" name="reject_verify" value="true">
                            @csrf
                            </form>
                          </div>
                        </div>
